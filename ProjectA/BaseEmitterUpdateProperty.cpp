...
void BaseEmitterUpdateProperty::Update(ID3D11DeviceContext* deviceContext, float dt)
{
	if (m_loopCount > 0)
	{
		if (m_emitterCurrentTime)
		{
			float& emitterCurrentTime = *m_emitterCurrentTime;
			emitterCurrentTime += dt;

			if (m_loopTime < emitterCurrentTime)
			{
				...
				{
					emitterCurrentTime = max(emitterCurrentTime - m_loopTime, 0.f);
					m_loopCount -= 1;
				}
			}
		}
