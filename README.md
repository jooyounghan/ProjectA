...
### 25.04.30 ~ 25.05.01
- 스프라이트 이미터에 대한 인덱스 보간 확인 완료
- 이미터마다 스프라이트에 활용하는 텍스처를 다르게 사용하기 위해 아래와 같이 구조 작성
	- HLSL 5.0의 경우 Texture2D에 대한 동적 인덱싱이 불가능하여 Texture2DArray(텍스쳐의 너비와 높이가 동일한 제한이 있음)를 스프라이트 이미터 매니저가 관리
	- 스프라이트 이미터가 텍스처 버퍼를 스프라이트 이미터 매니저로 전달하면 이를 통해 Texture2DArray의 EmitterID 인덱스에 텍스처 저장
	- 스프라이트 깊이 기준 정렬 코드 작성 시작 

### 25.05.02 ~ 25.05.06
- Radix Sort 구현
	- LSB부터 8비트씩 스캔하며 32비트 데이터(float)을 정렬하도록 구현
	- 이전 비트에 대한 정렬 결과가 유지되도록 하는 Stable 정렬로 구현
- Bloom 효과 구현
	- 현재는 하나의 렌더 타겟에 그리고 이에 대한 후처리를 동일하게 처리 중이나 파티클 이미터 /  스프라이트 이미터 후처리 기법 분리를 위해 구조 변경 예정

### 25.05.07
- Radix Sort 관련 시간 확인 결과 병목 지점 확인
	- global offset을 구하는 구간 내 그룹을 순회하는 시간이 크게 소모되어 수정 필요
...
### 25.05.15
- Reinhard시 화면의 밝기를 바탕으로 동적으로 ToneMapping 되도록 보완
- Radix Sort 보완 진행 중

### 25.05.16 ~ 25.05.18
- Radix Sort 보완 관련 추가 수행
	- 불필요한 읽기 / 쓰기  삭제
